...
internal sealed class FishingRuntimeContext
{
    ...
    // Mirrors the Tracking1 phase split:
    // - active reel metrics => TRACKING
    // - shake button visible => SHAKE
    // - otherwise => CASTING
    public bool TryGetTracking1State(out string phase, out ReelContext? reelContext, out ReelMetrics? metrics)
    {
        reelContext = GetRefStyleReelContext();
        metrics = reelContext is null ? null : ReadMetrics(reelContext);
    ...
    public ReelContext? GetRefStyleReelContext()
    {
        var reelGui = GetReelGui();
        ...
        var context = BuildRefStyleReelContextFromGui(reelGui);
        if (context is null)
        {
            ResetReelCache();
            return null;
    ...
    private ReelContext? BuildRefStyleReelContextFromGui(ulong reelGui)
    {
        var fish = _memory.FindDescendantByName(reelGui, "fish");
        var playerbar = _memory.FindDescendantByName(reelGui, "playerbar");
        if (fish == 0 || playerbar == 0)
        {
            return null;
        }

        var container = _memory.ReadParent(playerbar);
        if (container == 0)
        {
            container = reelGui;
        }

        var fishBounds = _memory.ReadGuiBounds(fish, false);
        var playerbarBounds = _memory.ReadGuiBounds(playerbar, false);
        var containerBounds = _memory.ReadGuiBounds(container, false);
        if (fishBounds is null || playerbarBounds is null || containerBounds is null)
        {
            return null;
        }
        ...
        if (containerBounds.Value.Width < 20 || containerBounds.Value.Height < 8)
        {
            return null;
        }

        var fishCenterX = fishBounds.Value.X + fishBounds.Value.Width * 0.5;
        var fishCenterY = fishBounds.Value.Y + fishBounds.Value.Height * 0.5;
        var playerbarCenterX = playerbarBounds.Value.X + playerbarBounds.Value.Width * 0.5;
        var playerbarCenterY = playerbarBounds.Value.Y + playerbarBounds.Value.Height * 0.5;
        if (!ContainsWithPadding(containerBounds.Value, fishCenterX, fishCenterY, 4) ||
            !ContainsWithPadding(containerBounds.Value, playerbarCenterX, playerbarCenterY, 4))
        {
            return null;
        }

        var bar = _memory.ReadParent(fish);
        if (bar == 0)
        {
            bar = container;
        }
    ...
    private static bool ContainsWithPadding(GuiBounds bounds, double x, double y, double padding)
    {
