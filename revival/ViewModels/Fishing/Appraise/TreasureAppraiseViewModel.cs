...
public sealed class TreasureAppraiseViewModel : ViewModelBase
{
    ...
    {
        AppLog.Info("MacroMode", "TreasureAppraise StartAsync requested.");
        AutoTreasureEnabled = true;
        _runner.Reset(ReadSettings());
        _rollingDotStep = 0;
        IsRunning = true;
        StatusText = "Rolling";
        AppLog.Info("MacroMode", $"TreasureAppraise started. delay={ClickDelaySecondsText} target={TargetWeightText}");
        return Task.CompletedTask;
    }

    public Task StopAsync()
    {
        AppLog.Info("MacroMode", "TreasureAppraise StopAsync requested.");
        IsRunning = false;
        _runner.Reset(ReadSettings());
        StatusText = "---";
        AppLog.Info("MacroMode", "TreasureAppraise stopped.");
        return Task.CompletedTask;
    }
