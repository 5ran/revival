...
int main()
{
    ...
    auto r1 = engine.Tick(RuntimeTrackerTickInput{1010, false, true, true, false, std::nullopt});
    failed += Assert(r1.phase != macro_port::FishingPhase::Off, "phase-casting");

    auto r2 = engine.Tick(RuntimeTrackerTickInput{1400, false, true, true, true, std::nullopt});
    failed += Assert(r2.phase != macro_port::FishingPhase::Off, "phase-running");

    // Ensure we enter fishing before testing completion recast.
    (void)engine.Tick(RuntimeTrackerTickInput{1600, false, true, false, true, std::nullopt});

    // Force completion path via override and verify recast reset.
    auto r3 = engine.Tick(RuntimeTrackerTickInput{1800, false, true, false, true, true});
