...
private:
    bool Open()
    {
        ...
        view_ = static_cast<const TrackingMetricsSharedState*>(
    ...
    HANDLE mapping_ = nullptr;
    const TrackingMetricsSharedState* view_ = nullptr;
};
...
private:
    ...
    void Run()
    {
        ...
        while (!stopRequested_.load()) {
            ...
            if (!memory_.EnsureAttached(&attachError)) {
                UpdateSharedStatus("WAITING", attachError.empty() ? "Waiting for Roblox." : attachError, std::nullopt);
            ...
            std::optional<std::uint64_t> csharpMetricAgeMs;
            std::optional<TrackingMetricsSnapshot> acceptedCsharpSnapshot;
            ...
            const auto csharpSnapshot = trackingMetricsShared_.TryRead();
            if (csharpSnapshot.has_value() && csharpSnapshot->tickMs > 0) {
                const auto ageMs = nowMs() >= csharpSnapshot->tickMs ? nowMs() - csharpSnapshot->tickMs : 0;
                csharpMetricAgeMs = ageMs;
                if (ageMs <= kTrackingMetricsFreshMs) {
                    metricSource = "csharp";
                    acceptedCsharpSnapshot = csharpSnapshot;
                    reelVisible = csharpSnapshot->reelVisible || csharpSnapshot->hasMetrics || csharpSnapshot->phase == 2;
                    shakeVisible = csharpSnapshot->shakeVisible || csharpSnapshot->phase == 1;
                    metrics = csharpSnapshot->hasMetrics
                        ? std::optional<macro_port::ReelMetrics>(csharpSnapshot->metrics)
                        : std::optional<macro_port::ReelMetrics>{};
                    progress = csharpSnapshot->progress;
                }
            }
            ...
            input.completionOverride = progress.has_value() ? std::optional<bool>(*progress >= 99.9) : std::nullopt;
            if (acceptedCsharpSnapshot.has_value() && acceptedCsharpSnapshot->hasDecision)
            {
                input.hasExternalTrackingDecision = true;
                input.externalDesiredHold = acceptedCsharpSnapshot->desiredHold;
                input.externalDecisionApplied = acceptedCsharpSnapshot->decisionApplied;
                input.externalDecisionMode = acceptedCsharpSnapshot->decisionMode;
                input.externalDecisionError = acceptedCsharpSnapshot->decisionError;
                input.externalDecisionControl = acceptedCsharpSnapshot->decisionControl;
            }
            ...
            if (logTick) {
                ...
                tickLog
                    ...
                    << " metricAge=" << (csharpMetricAgeMs.has_value() ? std::to_string(*csharpMetricAgeMs) : std::string("null"))
                    << " csharpPhase=" << (csharpSnapshot.has_value() ? std::to_string(csharpSnapshot->phase) : std::string("null"))
                    << " csharpHasMetrics=" << (csharpSnapshot.has_value() ? boolText(csharpSnapshot->hasMetrics) : std::string("null"))
                    << " csharpFish=" << (csharpSnapshot.has_value() && csharpSnapshot->hasMetrics ? std::to_string(csharpSnapshot->metrics.fishCenter) : std::string("null"))
                    << " csharpBar=" << (csharpSnapshot.has_value() && csharpSnapshot->hasMetrics ? std::to_string(csharpSnapshot->metrics.playerbarCenter) : std::string("null"))
                    << " csharpWidth=" << (csharpSnapshot.has_value() && csharpSnapshot->hasMetrics ? std::to_string(csharpSnapshot->metrics.playerbarWidth) : std::string("null"))
                    << " csharpDecision=" << (csharpSnapshot.has_value() ? boolText(csharpSnapshot->hasDecision) : std::string("null"))
                    << " csharpHold=" << (csharpSnapshot.has_value() && csharpSnapshot->hasDecision ? boolText(csharpSnapshot->desiredHold) : std::string("null"))
                    << " csharpApplied=" << (csharpSnapshot.has_value() && csharpSnapshot->hasDecision ? boolText(csharpSnapshot->decisionApplied) : std::string("null"))
                    << " csharpDecisionMode=" << (csharpSnapshot.has_value() && csharpSnapshot->hasDecision ? std::to_string(csharpSnapshot->decisionMode) : std::string("null"))
                    << " csharpDecisionError=" << (csharpSnapshot.has_value() && csharpSnapshot->decisionError.has_value() ? optionalDoubleText(csharpSnapshot->decisionError) : std::string("null"))
                    << " csharpDecisionControl=" << (csharpSnapshot.has_value() && csharpSnapshot->decisionControl.has_value() ? optionalDoubleText(csharpSnapshot->decisionControl) : std::string("null"))
            ...
            UpdateSharedStatus(phaseToString(result.phase), result.message, progress, metrics, reelContext);
        ...
        const std::optional<macro_port::ReelContext>& reelContext = std::optional<macro_port::ReelContext>{})
    {
        ...
        if (metrics.has_value()) {
            state_.settings["InputStatus"] = "Tracking";
        } else if (phase == "CASTING") {
            state_.settings["InputStatus"] = "Casting";
        } else if (phase == "SHAKE") {
            state_.settings["InputStatus"] = "Shaking";
        } else if (phase == "TRACKING") {
            state_.settings["InputStatus"] = "Tracking";
        } else {
            state_.settings["InputStatus"] = message;
        }
...
macro_port::RuntimeTrackerEngineSettings::TrackingMode parseTrackingMode(const std::string& value) {
    ...
    if (_stricmp(value.c_str(), "Predict") == 0 || _stricmp(value.c_str(), "Tracking 1") == 0 || _stricmp(value.c_str(), "Tracking1") == 0) {
...
std::string statusJson(const CoreState& state) {
    ...
    const auto hotkey = readSetting(state, "Hotkey", "F6");
