...
internal sealed class AquariumSequenceRunner
{
	...
	private double nextFeedClickTime;
	...
	public void Reset()
	{
		...
		feedUntilTime = 0.0;
		nextScrollTime = 0.0;
		nextFeedClickTime = 0.0;
		didFastScrollUp = false;
		aquariumButton = 0uL;
		feedAnchor = 0uL;
		playerGuiAddress = 0uL;
		aquariumPath = null;
		feedPath = null;
		infoLabel = 0uL;
		infoPath = null;
	...
	public AquariumSequenceResult Step(double clickDelaySeconds)
	{
		...
		if (phase == AquariumSequencePhase.WaitAfterOpen)
		{
			...
			NativeMouse.MoveTo(point.X, point.Y);
			feedUntilTime = totalSeconds + 3.333;
			nextScrollTime = totalSeconds;
			nextFeedClickTime = totalSeconds;
			didFastScrollUp = false;
		...
		if (phase == AquariumSequencePhase.Feeding)
		{
			...
			if (totalSeconds >= nextFeedClickTime)
			{
				NativeMouse.MoveTo(cursor.X, cursor.Y);
				NativeMouse.LeftDown();
				NativeMouse.LeftUp();
				nextFeedClickTime = totalSeconds + 0.05;
			}
...
internal static class NativeMouse
{
	...
	{
		mouse_event(MouseEventWheel, 0u, 0u, unchecked((uint)-120), UIntPtr.Zero);
	}

	public static void ScrollUp()
	{
		mouse_event(MouseEventWheel, 0u, 0u, 120u, UIntPtr.Zero);
	}
	...
	private static void MoveToWithJitter(int x, int y)
	{
		...
		mouse_event(MouseEventMove, 1u, 0u, 0u, UIntPtr.Zero);
		System.Threading.Thread.Sleep(8);
		mouse_event(MouseEventMove, unchecked((uint)-1), 0u, 0u, UIntPtr.Zero);
		System.Threading.Thread.Sleep(8);
		MoveTo(x, y);
		System.Threading.Thread.Sleep(8);
	}
