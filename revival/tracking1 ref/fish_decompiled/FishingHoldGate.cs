...
internal sealed class FishingHoldGate
{
    ...
    public FishingHoldAction Decide(bool desired, long nowMs, int delayMs, int refreshMs)
    {
        if (desired == _held)
        {
            if (desired && refreshMs > 0 && _lastActionAt != 0 && nowMs - _lastActionAt >= refreshMs)
            {
                _lastActionAt = nowMs;
                return FishingHoldAction.Press;
            }
