...
public sealed class TrackingOverlayService : IDisposable
{
    ...
    private const int MetricsInputHeldOffset = 80;
    private const int MetricsDecisionModeOffset = 84;
    ...
    private long lastTrackingInputTraceMs;
    private string lastTrackingInputState = string.Empty;
    ...
    public bool TryReadTrackingMetricsSnapshot(out TrackingMetricsSnapshot snapshot)
    {
        snapshot = default;
        ...
        {
            uint sequenceStart = trackingMetricsAccessor!.ReadUInt32(SequenceOffset);
            if ((sequenceStart & 1) != 0)
            {
                continue;
            }

            uint magic = trackingMetricsAccessor.ReadUInt32(MagicOffset);
            if (magic != TrackingMetricsMagic)
            {
                return false;
            }

            snapshot = new TrackingMetricsSnapshot(
                trackingMetricsAccessor.ReadUInt32(MetricsPhaseOffset),
                trackingMetricsAccessor.ReadUInt32(MetricsHasMetricsOffset) != 0,
                trackingMetricsAccessor.ReadUInt64(MetricsTickMsOffset),
                trackingMetricsAccessor.ReadDouble(MetricsFishCenterOffset),
                trackingMetricsAccessor.ReadDouble(MetricsPlayerbarCenterOffset),
                trackingMetricsAccessor.ReadDouble(MetricsPlayerbarWidthOffset),
                trackingMetricsAccessor.ReadDouble(MetricsProgressOffset),
                trackingMetricsAccessor.ReadUInt32(MetricsHasProgressOffset) != 0,
                trackingMetricsAccessor.ReadUInt32(MetricsShakeVisibleOffset) != 0,
                trackingMetricsAccessor.ReadUInt32(MetricsReelVisibleOffset) != 0,
                trackingMetricsAccessor.ReadUInt32(MetricsHasDecisionOffset) != 0,
                trackingMetricsAccessor.ReadUInt32(MetricsInputHeldOffset) != 0);

            uint sequenceEnd = trackingMetricsAccessor.ReadUInt32(SequenceOffset);
    ...
    public void TraceTrackingInputState(string inputState, ulong tickMs)
    {
        if (inputState == lastTrackingInputState && Environment.TickCount64 - lastTrackingInputTraceMs < 150)
        {
            return;
        }

        lastTrackingInputState = inputState;
        lastTrackingInputTraceMs = Environment.TickCount64;
        Trace($"tracking.input state={inputState} tickMs={tickMs}");
    }
    ...
    public readonly record struct TrackingMetricsSnapshot(
        uint Phase,
        bool HasMetrics,
        ulong TickMs,
        double FishCenter,
        double PlayerbarCenter,
        double PlayerbarWidth,
        double Progress,
        bool HasProgress,
        bool ShakeVisible,
        bool ReelVisible,
