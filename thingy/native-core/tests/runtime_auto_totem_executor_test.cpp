...
{
public:
    void RequestStart(std::int64_t) override
    {
        ++starts;
    }

    AutoTotemWorkflowInput::Result PollResult(std::int64_t) override
    {
        ++polls;
        return polls >= 2 ? AutoTotemWorkflowInput::Result::Success : AutoTotemWorkflowInput::Result::None;
    }
...
int main()
{
    ...
    RuntimeTrackerEngine engine(&runtime, &orchestrator, &controller, &a, &hold, s, &exec);

    engine.Start(1000, FishingCastingMode::Normal);
    auto t1 = engine.Tick(RuntimeTrackerTickInput{
        1100, false, false, false, false, std::nullopt, true, true, true, AutoTotemWorkflowInput::Result::None});
    (void)t1;

    auto t2 = engine.Tick(RuntimeTrackerTickInput{
        1200, false, false, false, false, std::nullopt, true, true, true, AutoTotemWorkflowInput::Result::None});
    (void)t2;
