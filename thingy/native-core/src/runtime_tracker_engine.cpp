...
RuntimeTrackerTickResult RuntimeTrackerEngine::Tick(const RuntimeTrackerTickInput& input)
{
    ...
    if (phaseOut.phase == FishingPhase::Fishing && metrics.has_value() && controller_ != nullptr)
    {
        ...
        const bool predictMode = settings_.mode == RuntimeTrackerEngineSettings::TrackingMode::Tracking1;
        ...
        if (freshReelTick)
        {
            ...
            startupAssistActive_ = settings_.startupAssistEnabled || predictMode;
        ...
        if (predictMode && startupAssistActive_)
        {
