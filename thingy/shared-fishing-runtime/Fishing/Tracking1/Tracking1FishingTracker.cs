...
public sealed class Tracking1FishingTracker : IFishingTracker
{
    ...
    private void UpdatePerfectCasting()
    {
        ...
        if (_castReadyAt != 0 && now < _castReadyAt)
        {
            Status = new FishingTrackerStatus(true, "CASTING", "Preparing cast.", null, false);
            return;
        }

        if (_completionRecastReadyAt != 0 && now < _completionRecastReadyAt)
        {
        ...
        if (now - _castStartedAt < _perfectCastingSettings.PreCastDelayMs)
        {
        ...
        if (_castReleasedAt == 0)
        {
            ...
            if (power is null)
            {
                if (now - _castStartedAt >= Math.Max(5000, _perfectCastingSettings.CastTimeoutMs))
                {
                    ResetCastingCycle();
                }
            ...
            Status = new FishingTrackerStatus(
                true,
                "CASTING",
                power is null ? "Waiting for cast power bar." : "Charging perfect cast.",
                null,
                true);

            if (power is { } currentPower && currentPower >= targetPower)
            {
                NativeMouse.LeftUp();
        ...
        if (_context.IsShakeButtonVisible())
        {
            if (_lastShakedAt == 0 || now - _lastShakedAt >= _perfectCastingSettings.ShakeIntervalMs)
            {
                NativeKeyboard.PressEnter();
                _lastShakedAt = now;
            }

            if (now - _castReleasedAt >= _perfectCastingSettings.CastTimeoutMs)
            {
                ResetCastingCycle();
            }
